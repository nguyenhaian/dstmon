(function() {
    ...
    TrackerController.$inject = ['$scope', '$http', '$timeout', 'socket', '$compile'];

    function TrackerController($scope, $http, $timeout, socket, $compile) {
        ...
        socket.getCCU({ time: "1h" }, function onSuccess(response) {
            // tsvdata: results.tsv,
            // rccu: results.rccu,
            // req: req

            // var tsvdata = '<%= tsvdata %>';
            var data = d3.tsvParse(response.tsvdata.replace(/[\t]/g," "));
            // var str = new String(response.tsvdata);
            // var newstr = Object.keys(str).map(function(key, index) {
            //     return str[key];
            // }).join("");

            // var data = d3.tsvParse(newstr);
            console.log("************ TSV PARSE ************");
            console.log(response.tsvdata.replace(/[\t]/g," "));
            console.log(data);

            $scope.req = response.req;
            var ccus = {};
