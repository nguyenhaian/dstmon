...
angular.module('rgkevin.datetimeRangePicker', ['vr.directives.slider'])
    .filter('rgTime', [function () {
         ...
         */
        return function (input, starttime, type) {
            // var
            ...
            // thực ra starttime is a moment instance
            return moment(starttime).add(input, 'minutes').format("DD-MM-YYYY HH:mm");
        };
    }])
	...
	return {
                            ...
                            '<div class="rg-range-picker-slider" id="rgRangePickerSliderContainer" ng-show="data.hasTimeSliders">' +
                                '<div class="rg-range-picker-slider-labels">' +
                                    '<div class="row">' +
                                        ...
                                        '<div class="col-xs-6 text-center"><span class="label label-range-picker">{{data.time.from | rgTime:data.time.startdatetime:data.time.hours24}}</span></div>' +
                                        '<div class="col-xs-6 text-center"><span class="label label-range-picker">{{data.time.to | rgTime:data.time.startdatetime:data.time.hours24}}</span></div>' +
                                    '</div>' +
                                '</div>' +
                            '</div>' +
                        // '</div>' +
		...
		link: function(scope, element, attrs) {
            // define labels
            var
                sliderMinWidth      = 720,// if directive is less width than 500, then display responsive version
